@testset "Test TrieNode creation and insertion" begin
    ...
    insert!(root, "topic1/device1", (topic, payload) -> println("Callback for $topic: $payload"))
    insert!(root, "topic1/device2", (topic, payload) -> println("Callback for $topic: $payload"))
    insert!(root, "topic2/+/foo", (topic, payload) -> println("Wildcard data for $topic: $payload"))
    insert!(root, "topic2/bar/foo", (topic, payload) -> println("Callback for $topic: $payload"))
    ...
    @test root.children["topic2"].children["+"].children["foo"].cb isa MQTTClient.FunctionCallback
    @test root.children["topic2"].children["bar"].children["foo"].cb isa MQTTClient.FunctionCallback
end
