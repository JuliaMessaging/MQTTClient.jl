...
tcp_test_client, tcp_test_conn = MakeConnection(MQTTClient.IOConnection(localhost, 1883))
try
    ...
    @testset "TCP Smoke tests" begin
        println("="^80)
        println("Running smoke tests against localhost")
        println("-"^80)
    ...
    @testset "TCP stress test" begin
        println("="^80)
        println("Running stress tests against localhost")
        println("-"^80)
