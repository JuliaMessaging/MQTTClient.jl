...
struct Connection{T<:AbstractIOConnection} <: AbstractConfigElement
    ...
    ) where {T<:AbstractIOConnection}
        return new{T}(protocol, keep_alive, client_id, user, will, clean_session)
    end
...
function Base.show(io::IO, connection::Connection)
    return print(
        io,
        "Connection(Protocol: $(connection.protocol), Client ID: $(connection.client_id)",
        (connection.user == User("", "") ? "" : ", User Name: $(connection.user.name)"),
        ")",
    )
end
...
struct Configuration
    ...
    function Configuration(
        ...
        return new(
            Client(ping_timeout),
            Connection(io, keep_alive, client_id, user, will, clean_session),
        )
    end
...
function Base.iterate(conf::Configuration, state=1)
    return state == 1 ? (conf.client, 2) : (conf.connection, nothing)
end
